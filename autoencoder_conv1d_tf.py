...
# 5. MODEL
# ─────────────────────────────────────────────
def build_conv1d_autoencoder(seq_len: int, n_features: int) -> Model:
    inputs = keras.Input(shape=(seq_len, n_features), name="input")

    # ── Encoder ──────────────────────────────
    x = layers.Conv1D(HIDDEN_UNITS, kernel_size=3, padding="same",
                      activation="relu", name="enc_conv_1")(inputs)
    x = layers.Dropout(DROPOUT_RATE, name="enc_dropout_1")(x)
    x = layers.Conv1D(HIDDEN_UNITS // 2, kernel_size=3, padding="same",
                      activation="relu", name="enc_conv_2")(x)
    x = layers.Dropout(DROPOUT_RATE, name="enc_dropout_2")(x)
    latent = layers.Conv1D(LATENT_DIM, kernel_size=3, padding="same",
                           activation="relu", name="latent")(x)

    # ── Decoder ──────────────────────────────
    x = layers.Conv1DTranspose(HIDDEN_UNITS // 2, kernel_size=3, padding="same",
                               activation="relu", name="dec_tconv_1")(latent)
    x = layers.Dropout(DROPOUT_RATE, name="dec_dropout_1")(x)
    x = layers.Conv1DTranspose(HIDDEN_UNITS, kernel_size=3, padding="same",
                               activation="relu", name="dec_tconv_2")(x)
    x = layers.Dropout(DROPOUT_RATE, name="dec_dropout_2")(x)
    output = layers.Conv1DTranspose(
        n_features, kernel_size=3, padding="same", activation="linear", name="reconstruction"
    )(x)

    model = Model(inputs, output, name="Conv1D_Autoencoder_Shared")
    model.compile(
...
# 8. PLOTTING
# ─────────────────────────────────────────────
def save_training_history(history, output_csv: str = "training_history_conv1d_vib.csv"):
...
def plot_results(history, errors_test, y_test, machine_ids_test, threshold):
    ...
    fig.suptitle("Conv1D Autoencoder — Shared Model, All Machines",
    ...
    plt.tight_layout()
    out_path = BASE_DIR / "anomaly_results_shared_conv1d_vib.png"
...
def plot_confusion_matrix_figure(y_true, errors_test, threshold,
                                 output_path: str = "confusion_matrix_shared_conv1d_vib.png"):
...
def main(filepath: str = "C:\\cudatest\\PFA\\predictive_maintenance_dataset.csv"):
    ...
    # Build & train model
    model = build_conv1d_autoencoder(SEQUENCE_LENGTH, len(feature_cols))
    model.summary()
    ...
    cb_list = [
        ...
        callbacks.ModelCheckpoint("best_model_conv1d_temp.keras", monitor="val_loss",
    ...
    save_training_history(history, "training_history_conv1d_vib.csv")
